...
class MailClassificationsController extends AppController {

    var $name = "MailClassifications";
        ...
        if ($this->request->is("post") || $this->request->is("put")) {
            ...
            if ($this->{ Inflector::classify($this->name) }->validates()) {
                if (!is_null($id)) {
                    ...
                    $this->MailClassification->data['MailClassification']['id'] = $id;
                    $this->{ Inflector::classify($this->name) }->saveAll($this->{ Inflector::classify($this->name) }->data, array('deep' => true));
    ...
    function view_mail_classification($id = null) {
        if (!$this->MailClassification->exists($id)) {
            throw new NotFoundException(__('Invalid Data'));
        } else {
            $this->autoRender = false;
            if($this->request->is("GET")) {
                $data = $this->MailClassification->find("first",["conditions" => ["MailClassification.id" => $id]]);
