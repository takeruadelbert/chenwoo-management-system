...
class AppHelper extends Helper {
    ...
    function dotNumberSeperator($number) {
        if ($number == "") {
            return "0";
        }
    ...
    function NominalSeparator($nominal, $currency_type) {
        if ($currency_type == 1) { // IDR
            return $this->IDR($nominal);
        } else {
            return $this->USD($nominal);
        }
    }

    /* API source : http://stackoverflow.com/questions/19838049/google-currency-converter-has-changed-its-url-but-not-getting-same-result */
    ...
    function getContact($no_hp, $no_phone) {
        if (!empty($no_hp) && !empty($no_phone)) {
            return $no_phone . " / " . $no_hp;
        } else if (!empty($no_hp) && empty($no_phone)) {
            return $no_hp;
        } else if (empty($no_hp) && !empty($no_phone)) {
            return $no_phone;
        } else {
    ...
    function cvtTanggalWaktu($date = null, $now = true) {
        if (!empty($date)) {
            $tgl = date("d", strtotime($date));
            $bulan = $this->getNamaBulan(date("m", strtotime($date)));
            $tahun = date("Y", strtotime($date));
        ...
        return "$tgl $bulan $tahun $hour:$minute:$second";
    }
    ...
    function cvtJam($date = null, $now = true) {
        if (!empty($date)) {
    ...
    function cvtWaktu($date = null, $now = true) {
        if (!empty($date)) {
            $tgl = date("d", strtotime($date));
            $bulan = $this->getNamaBulan(date("m", strtotime($date)));
            $tahun = date("Y", strtotime($date));
    ...
    function cvtBulanTahun($Ym = false) {
        if ($Ym === false) {
            return "-";
        } else {
            return $this->getBulan($Ym, true) . "-" . $this->getTahun($Ym, true);
        }
    }
    ...
    function toHHMMSS($seconds = 0, $strip = false) {
        $t = round($seconds);
        if ($strip === true && $seconds === 0) {
            return "-";
        } else {
