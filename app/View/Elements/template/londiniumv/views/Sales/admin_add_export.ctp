<?php echo $this->Form->create("Sale", array("class" => "form-horizontal form-separate", "action" => "add_export", "type" => "file", "id" => "formSubmit", "inputDefaults" => array("error" => array("attributes" => array("wrap" => "label", "class" => "error"))))) ?>
<div class="tab-pane active fade in" id="data-utama">
    <div class="panel panel-default">
        <div class="panel-body">
            <div class="block-inner text-danger">
                <h6 class="heading-hr"><?= __("Tambah Penjualan Produk Eksport") ?>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control text-right" name="data[Sale][shipping_cost]">
                                        </div>
                                    </div>
                                </div>    
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        <div class="input-group">
                                            ...
                                            <input type="text" class="form-control text-right isdecimal" name="data[Sale][exchange_rate]" required="required" value =<?= ic_rupiah($this->Html->getExchangeRate(1, "USD", "IDR")) ?>>
                                            <span class="input-group-btn">
...
<script>
    ...
    function updateTotal(n) {
        $("#mcweight" + n).on("change", function () {
            var grandTotal = 0;
            var price = 0;
            if ($("input.SaleDetail" + n + "Price").val() != "") {
                price = parseFloat($("input.SaleDetail" + n + "Price").val());
            }
            var weight = 0;
            if ($("#lbs" + n).val() != "") {
                weight = ac_number_reverse($("#lbs" + n).val());
            }
//            if ($("#mcweight" + n).val() == "") {
//                weight = 0;
//            }
            var total = price * weight;
            $("input.SaleDetail" + n + "Total").val(ac_dollar(total));
            $.each($(".subtotal"), function () {
                grandTotal += ac_number_reverse($(this).val());
            })
        ...
        $("input.SaleDetail" + n + "Quantity").on("change keyup", function () {
            ...
            var price = 0;
            if ($("input.SaleDetail" + n + "Price").val() != "") {
                price = ac_number_reverse($("input.SaleDetail" + n + "Price").val());
            }
            var weight = 0;
            if ($("#lbs" + n).val() != "") {
                weight = parseFloat($("#lbs" + n).val().replace(",", ""));
            }
            var total = price * weight;
            $("input.SaleDetail" + n + "Total").val(ac_dollar(total));
            $.each($(".subtotal"), function () {
    ...
    function updateJumlahBerat(n) {
        $("#mcweight" + n).on("change", function () {
            ...
            total = $thisTotal;
            $thisTotalLbs = ac_lbs(quantity * mc);
...
<script type="x-tmpl-mustache" id="tmpl-product-data">
    ...
    <div class="input-group">
    <input type='text' name='data[SaleDetail][{{n}}][quantity]' class='form-control SaleDetail{{n}}Quantity qtyIdx text-right quantityWeight isdecimal' id='SaleDetail{{n}}Quantity'/>
    ...
    <div class="input-group">
    <span class="input-group-addon">$</span>        
    <input type='text' name="data[SaleDetail][{{n}}][price]" class='form-control SaleDetail{{n}}Price qtyPrice text-right' value="0" id="SaleDetail{{n}}Price" onkeyup="updateTotal({{n}})"/>
    <div>        
    </td>  
    <td class="text-center">
    <div class="input-group">        
