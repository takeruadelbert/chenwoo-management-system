<?php
echo $this->element(_TEMPLATE_DIR . "/{$template}/filter/cooperative-cash-receipt");
?>
<div class="panel panel-default">
    <div class="panel-body">
        <div class="block-inner text-danger">
            <h6 class="heading-hr"><?= __("DATA TRANSAKSI PENJUALAN KOPERASI") ?>
                            ...
                            foreach ($data['rows'] as $item) {
                                ...
                                <tr id="row-<?= $i ?>" class="removeRow<?php echo $item[Inflector::classify($this->params['controller'])]['id']; ?>">
                                    ...
                                    <td class="text-center">
                                        <a data-toggle="modal" data-target="#default-view-coop-transaction" href="<?= Router::url("/admin/popups/content?content=viewcooptransaction-pnjl&id={$item[Inflector::classify($this->params['controller'])]['id']}") ?>" class="ajax-modal"><?= $item['CooperativeCashReceipt']['reference_number'] ?></a>                                           
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <a href="<?= Router::url("/admin/{$this->params['controller']}/print_cooperative_cash_receipt/{$item[Inflector::classify($this->params['controller'])]['id']}") ?>" target="_blank"><button type="button" class="btn btn-default btn-xs btn-icon tip" title="Cetak Transaksi Penjualan"><i class="icon-print"></i></button></a>
                                    </td>
                                </tr>
                                <?php
                                $i++;
