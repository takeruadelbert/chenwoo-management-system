<?php
echo $this->element(_TEMPLATE_DIR . "/{$template}/filter/absensi-kehadiran");
if (false) {
    ?>
        ...
        <div class = "panel-body">
            <div class = "block-inner text-danger">
                <h6 class = "heading-hr">DATA ABSENSI PEGAWAI <div class = "pull-right">
                        <button class = "btn btn-xs btn-default" type = "button" onclick="exp('print', '<?php echo Router::url("absensi_kehadiran/print?" . $_SERVER['QUERY_STRING'], true) ?>')"><i class = "icon-print2"></i> Cetak</button>&nbsp;
                        <button class = "btn btn-xs btn-default" type = "button" onclick="exp('excel', '<?php echo Router::url("absensi_kehadiran/excel?" . $_SERVER['QUERY_STRING'], true) ?>')"><i class = "icon-file-excel"></i> Excel</button>&nbsp;
                    </div>
                    <small class = "display-block"><?= _APP_CORPORATE_FULL ?></small>
                </h6>
            </div>
                        ...
                        <tbody>
                            ...
                            foreach ($empData as $empId => $item) {
                                ...
                                <tr>
                                    ...
                                    <td width = "3%" align = "center" valign = "middle" class = "text-center"><a href = "<?= Router::url($item['Account']['User']['profile_picture'], true) ?>" class = "lightbox" title = "<?= $item['Account']['Biodata']['full_name'] ?>"><img src = "<?= Router::url($item['Account']['User']['profile_picture'], true) ?>" alt = "" class = "img-media"></a></td>
                                    ...
                                    <td width = "2%" class = "text-center">
                                        <a data-toggle="modal" data-remote="false" role="button" href="<?= Router::url("/admin/popups/content?content=detailkehadiran&employee_id=$empId&start=" . $start_date . "&end=" . $end_date) ?>" data-target="#default-modalkehadiran" class="btn btn-default btn-xs btn-icon tip" title="Lihat Data"><i class="icon-file6"></i></a>
                                    </td>
                                </tr>
                                <?php
