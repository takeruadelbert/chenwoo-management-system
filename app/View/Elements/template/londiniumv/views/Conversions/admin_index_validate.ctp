<?php
echo $this->element(_TEMPLATE_DIR . "/{$template}/filter/conversion-validate");
?>
<div class="panel panel-default">
    <div class="panel-body">
        <div class="block-inner text-danger">
            <h6 class="heading-hr"><?= __("DATA KONVERSI YANG TIDAK MEMENUHI RATIO") ?>
                            ...
                            foreach ($data['rows'] as $item) {
                                ...
                                <tr id="row-<?= $i ?>" class="removeRow<?php echo $item["Conversion"]['id']; ?>">
                                    ...
                                    <td class="text-center"  id = "target-change-status<?= $i ?>">
                                        <?php
                                        if ($item['Conversion']['verify_status_id'] == 2) {
                                            echo "Ditolak";
                                        } else if ($item['Conversion']['verify_status_id'] == 3) {
                                            echo "Disetujui";
                                        } else {
                                            echo $this->Html->changeStatusSelect($item['Conversion']['id'], ClassRegistry::init("VerifyStatus")->find("list", array("fields" => array("VerifyStatus.id", "VerifyStatus.name"))), $item['Conversion']['verify_status_id'], Router::url("/admin/conversions/change_status_verify"), "#target-change-status$i");
                                        }
                                        ?>
                                    </td>
