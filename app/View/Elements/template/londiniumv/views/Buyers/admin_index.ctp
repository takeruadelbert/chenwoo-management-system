<?php
echo $this->element(_TEMPLATE_DIR . "/{$template}/filter/buyer");
?>
<div class="panel panel-default">
    <div class="panel-body">
        <div class="block-inner text-danger">
            <h6 class="heading-hr"><?= __("DATA PEMBELI") ?>
                        ...
                        if (empty($data['rows'])) {
                            ...
                            foreach ($data['rows'] as $item) {
                                ...
                                <tr id="row-<?= $i ?>" class="removeRow<?php echo $item[Inflector::classify($this->params['controller'])]['id']; ?>">
                                    ...
                                    <td class="text-left"><?= $this->Echo->empty_strip($item['Buyer']['company_name']) ?></td>
                                    <td class="text-center"><?= $this->Echo->empty_strip($item['Buyer']['company_uniq_name']) ?></td>
                                    <td class="text-left"><?= $this->Echo->empty_strip($item['Buyer']['address']) ?></td>
                                    <td class="text-left"><?= $this->Echo->empty_strip($item['State']['name']) ?></td>
                                    <td class="text-left"><?= $this->Echo->empty_strip($item['City']['name']) ?></td>
                                    <td class="text-left"><?= $this->Echo->empty_strip($item['Country']['name']) ?></td>
                                    <td class="text-left"><?= $this->Echo->empty_strip($item['Buyer']['phone_number']) ?></td>
                                    <td class="text-left"><?= $this->Echo->empty_strip($item['Buyer']['email']) ?></td>
                                    <td class="text-left"><?= $this->Echo->empty_strip($item['Buyer']['website']) ?></td>
