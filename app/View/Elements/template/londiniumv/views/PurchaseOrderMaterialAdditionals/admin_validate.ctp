<?php
echo $this->element(_TEMPLATE_DIR . "/{$template}/filter/po_material_additional_validate");
?>
<div class="panel panel-default">
    <div class="panel-body">
        <div class="block-inner text-danger">
            <h6 class="heading-hr"><?= __("VALIDASI PURCHASE ORDER MATERIAL PEMBANTU") ?>
                            ...
                            foreach ($data['rows'] as $item) {
                                ...
                                <tr id="row-<?= $i ?>" class="removeRow<?php echo $item["PurchaseOrderMaterialAdditional"]['id']; ?>">
                                    ...
                                    <td class="text-center"  id = "target-change-status<?= $i ?>">
                                        <?php
                                        if ($item['PurchaseOrderMaterialAdditional']['purchase_order_material_additional_status_id'] == 3) {
                                            echo "Ditolak";
                                        } else if ($item['PurchaseOrderMaterialAdditional']['purchase_order_material_additional_status_id'] == 2) {
                                            echo "Disetujui";
                                        } else {
                                            echo $this->Html->changeStatusSelect($item['PurchaseOrderMaterialAdditional']['id'], ClassRegistry::init("PurchaseOrderMaterialAdditionalStatus")->find("list", array("fields" => array("PurchaseOrderMaterialAdditionalStatus.id", "PurchaseOrderMaterialAdditionalStatus.name"))), $item['PurchaseOrderMaterialAdditional']['purchase_order_material_additional_status_id'], Router::url("/admin/purchase_order_material_additionals/change_status_verify"), "#target-change-status$i");
                                        }
                                        ?>
                                    </td>
