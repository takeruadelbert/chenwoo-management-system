<?php echo $this->Form->create("PaymentSale", array("class" => "form-horizontal form-separate", "action" => "add_export", "id" => "formSubmit", "inputDefaults" => array("error" => array("attributes" => array("wrap" => "label", "class" => "error"))))) ?>
<div class="panel panel-default">
    <div class="panel-body">
        <div class="block-inner text-danger">
            <h6 class="heading-hr"><?= __("FORM INVOICE PENJUALAN EKSPORT") ?>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-4">
                                                <div class="input-group">
                                                    ...
                                                    <input type="text" class="form-control text-right isdecimal" readonly id="invoice">
                                                    <input type="hidden" class="form-control text-right isdecimal" readonly id="invoiceHidden" name="data[PaymentSale][total_invoice_amount]">
                                                </div>
                                            </div> 
                                            <div class="col-sm-2 control-label">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-4">
                                                <div class="input-group">
                                                    ...
                                                    <input type="text" class="form-control text-right" name="data[PaymentSale][amount]" required="required">
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="3" style="width:200px">
                                        <div class="form-group">
                                            ...
                                            echo $this->Form->input("PaymentSale.initial_balance_id", array("div" => array("class" => "col-sm-4"), "label" => false, "class" => "select-full", "data-placeholder" => "- Pilih Kas -", "empty" => ""));
                                            ?>
                                            <?php
                                    ...
                                    <table width="100%" class="table table-hover table-bordered">                        
                                        <thead>
                                            <tr>
                                                ...
                                                <th colspan="2"><?= __("Jumlah Produk (MC)") ?></th>
                                                <th colspan="2"><?= __("Isi Per Mc") ?></th>
                                                <th colspan="2"><?= __("Jumlah (Lbs)") ?></th>
                                                <th colspan="2"><?= __("Total Berat") ?></th>
                                                <th colspan="2"><?= __("Harga Produk ($ / Lbs)") ?></th>
...
<script>
    ...
    $(document).ready(function () {
        var sale = new Bloodhound({
            ...
            prefetch: '<?= Router::url("/admin/sales/list_hutang/2", true) ?>',
            remote: {
                url: '<?= Router::url("/admin/sales/list_hutang/2", true) ?>' + '?q=%QUERY',
                wildcard: '%QUERY',
            }
        });
        ...
        $('input.typeahead-ajax-transaction').typeahead({
            ...
            templates: {
                ...
                suggestion: function (context) {
                    return '<p>Nomor Invoice : ' + context.sale_no + '<br>Total Tagihan : $ ' + ac_dollar(context.total_invoice) + '</p>';
                },
                empty: [
        ...
        $('input.typeahead-ajax-transaction').bind('typeahead:select', function (ev, suggestion) {
            ...
            $('#invoice').val(ac_dollar(suggestion.total_invoice));
            $('#invoiceHidden').val((suggestion.total_invoice));
            $('#shipmentPaymentType').val(suggestion.shipment_payment_type);
            $('#shipmentPaymentTypeId').val(suggestion.shipment_payment_type_id);
            $('#shippingCost').val(ac_dollar(suggestion.shipping_cost));
                ...
                if (data != null && data != '') {
                    ...
                    $.each(data, function (index, item) {
                        var options = {
                            ...
                            quantity: item["SaleDetail"]["quantity"],
                            mc_weight: item["McWeight"]["lbs"],
                            total_lbs: item["SaleDetail"]["quantity"] * item["McWeight"]["lbs"],
                            weight: ic_kg(item["SaleDetail"]["nett_weight"]),
                            price: ac_dollar(item["SaleDetail"]["price"]),
                            prices: ac_dollar((item["SaleDetail"]["price"] * item["SaleDetail"]["quantity"] * item["McWeight"]["lbs"]).toFixed(2)),
                        };
                        if (data[0]["Sale"]["shipment_payment_type_id"] == 1) {
                            $("#sub_total").hide();
                            $("#shipping_cost").hide();
                            total += item["SaleDetail"]["price"] * item["SaleDetail"]["quantity"] * item["McWeight"]["lbs"];
                            $('.auto-calculate-grand-total').text(ac_dollar(total.toFixed(2)));
                        } else {
                            $("#sub_total").show();
                            total += item["SaleDetail"]["price"] * item["SaleDetail"]["quantity"] * item["McWeight"]["lbs"];
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data != null && data != '') {
                        ...
                        $("#invoiceLeft").val(ac_dollar(remainingAmount));
                    } else {
                        result = totalAmount - remainingAmount;
                        $("#invoiceLeft").val(ac_dollar(result));
                    }
                }
            });
        } else {
            $("#invoiceLeft").val(ac_dollar(totalInvoice));
        }
    }
</script>
