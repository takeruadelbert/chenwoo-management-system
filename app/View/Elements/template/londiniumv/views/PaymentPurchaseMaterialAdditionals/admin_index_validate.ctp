<?php
echo $this->element(_TEMPLATE_DIR . "/{$template}/filter/payment-purchase-material-additional-validate");
?>
<div class="panel panel-default">
    <div class="panel-body">
        <div class="block-inner text-danger">
            <h6 class="heading-hr"><?= __("VALIDASI PEMBELIAN MATERIAL PEMBANTU") ?>
                            ...
                            foreach ($data['rows'] as $item) {
                                ...
                                <tr id="row-<?= $i ?>" class="removeRow<?php echo $item[Inflector::classify($this->params['controller'])]['id']; ?>">
                                    ...
                                    <td class="text-center"  id = "target-change-status<?= $i ?>">
                                        <?php
                                        if ($item['PaymentPurchaseMaterialAdditional']['verify_status_id'] == 2) {
                                            echo "Ditolak";
                                        } else if ($item['PaymentPurchaseMaterialAdditional']['verify_status_id'] == 3) {
                                            echo "Disetujui";
                                        } else {
                                            echo $this->Html->changeStatusSelect($item['PaymentPurchaseMaterialAdditional']['id'], ClassRegistry::init("VerifyStatus")->find("list", array("fields" => array("VerifyStatus.id", "VerifyStatus.name"))), $item['PaymentPurchaseMaterialAdditional']['verify_status_id'], Router::url("/admin/payment_purchase_material_additionals/change_status_verify"), "#target-change-status$i");
                                        }
                                        ?>
                                    </td>
