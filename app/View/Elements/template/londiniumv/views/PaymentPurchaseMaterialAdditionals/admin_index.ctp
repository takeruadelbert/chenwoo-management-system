<?php
echo $this->element(_TEMPLATE_DIR . "/{$template}/filter/payment-purchase-material-additional");
?>
<div class="panel panel-default">
    <div class="panel-body">
        <div class="block-inner text-danger">
            <h6 class="heading-hr"><?= __("DATA PEMBELIAN MATERIAL PEMBANTU") ?>
                            ...
                            foreach ($data['rows'] as $item) {
                                ...
                                <tr id="row-<?= $i ?>" class="removeRow<?php echo $item[Inflector::classify($this->params['controller'])]['id']; ?>">
                                    ...
                                    <td class="text-center">
                                        <a data-toggle="modal" data-target="#default-view-coop-transaction" role="button" href="<?= Router::url("/admin/popups/content?content=viewpaymentpurchase-materialpembantu&id={$item[Inflector::classify($this->params['controller'])]['id']}") ?>"><button type="button" class="btn btn-default btn-xs btn-icon tip" title="Lihat Data"><i class="icon-eye7"></i></button></a>
                                        <?php
                                        if ($item['PaymentPurchaseMaterialAdditional']['verify_status_id'] == 3) {
                                            ?>
                                            <a target = "_blank" href = "<?= Router::url("/admin/{$this->params['controller']}/print_purchase_material_additional_receipt/{$item[Inflector::classify($this->params['controller'])]['id']} ", true) ?>" class = "btn btn-default btn-xs btn-icon btn-icon tip" title = "Cetak Kwitansi Pembelian"><i class = "icon-print2"></i></a>
                                            <?php
                                        } else {
                                            ?>
                                            <a target = "_blank" href = "" class = "btn btn-default btn-xs btn-icon btn-icon tip" title = "Cetak Kwitansi Pembelian" disabled><i class = "icon-print2"></i></a>
                                                <?php
                                            }
                                            ?>
                                    </td>
