<?php echo $this->Form->create("EmployeeSalary", array("class" => "form-horizontal form-separate", "action" => "add_bulanan", "id" => "formSubmit", "inputDefaults" => array("error" => array("attributes" => array("wrap" => "label", "class" => "error"))))) ?>
<div class="tab-pane active fade in" id="data-utama">
    <div class="panel panel-default">
        <div class="panel-body">
            <div class="block-inner text-danger">
                <h6 class="heading-hr"><?= __("FORM GAJI PEGAWAI BULANAN") ?>
...
<script>
    var employee = new Bloodhound({
        ...
        prefetch: '<?= Router::url("/admin/employees/list_bulanan/2", true) ?>',
        remote: {
            url: '<?= Router::url("/admin/employees/list_bulanan/2", true) ?>' + '?q=%QUERY',
            wildcard: '%QUERY',
        }
    });
...
//        addThisRow(".debt", "debt");
    ...
    function fastTemplate(data, t, target) {
    ...
    function getSalaryDetail() {
        ...
        $.ajax({
            type: "GET",
            dataType: "JSON",
            url: BASE_URL + "admin/employees/get_salary_detail?employee_id=" + employeeId + "&start_date=" + startDate + "&end_date=" + endDate,
            ...
            success: function (response) {
                ...
                $.each(response.data.relation.EmployeeSalaryLoan, function (k, v) {
                    v["h"]=k;
                    fastTemplate(v,"loan","#basket");
                });
                $.each(response.data.relation.EmployeeSalarySaleProductAdditional, function (k, v) {
                    v["h"]=k;
                    fastTemplate(v,"sale_product_additional","#basket");
                });
                fastTemplate(response.data.relation.EmployeeSalaryItemLoan,"item_loan","#basket");
                calculatetotalincome();
                calculatetotaldebt();
                reloadisdecimal();
                ajaxLoadingSuccess();
...
<script type="x-tmpl-mustache" id="tmpl-sale_product_additional">
    <input type="hidden" name="data[EmployeeSalarySaleProductAdditional][{{h}}][sale_product_additional_id]" value="{{id}}"/>
</script>
<script type="x-tmpl-mustache" id="tmpl-loan">
    <input type="hidden" name="data[EmployeeSalaryLoan][{{h}}][employee_data_loan_id]" value="{{id}}"/>
    <input type="hidden" name="data[EmployeeSalaryLoan][{{h}}][amount]" value="{{amount}}"/>
</script>
<script type="x-tmpl-mustache" id="tmpl-item_loan">
    <input type="hidden" name="data[EmployeeSalaryItemLoan][cooperative_item_loan_payment_detail_id]" value="{{cooperative_item_loan_payment_detail_id}}"/>
    <input type="hidden" name="data[EmployeeSalaryItemLoan][cooperative_item_loan_id]" value="{{cooperative_item_loan_id}}"/>
    <input type="hidden" name="data[EmployeeSalaryItemLoan][amount]" value="{{amount}}"/>
</script>