<?php
echo $this->element(_TEMPLATE_DIR . "/{$template}/filter/purchase-invoice-validate");
?>
<div class="panel panel-default">
    <div class="panel-body">
        <div class="block-inner text-danger">
            <h6 class="heading-hr"><?= __("VALIDASI PEMBELIAN") ?>
                        ...
                        if (empty($data['rows'])) {
                            ...
                            foreach ($data['rows'] as $item) {
                                ...
                                <tr id="row-<?= $i ?>" class="removeRow<?php echo $item[Inflector::classify($this->params['controller'])]['id']; ?>">
                                    ...
                                    <td class="text-center"><?= $this->Html->cvtTanggal($item['PaymentPurchase']['payment_date']) ?></td>
                                    ...
                                    <td class="text-center"  id = "target-change-status<?= $i ?>">
                                        <?php
                                        if ($item['PaymentPurchase']['verify_status_id'] == 2) {
                                            echo "Ditolak";
                                        } else if ($item['PaymentPurchase']['verify_status_id'] == 3) {
                                            echo "Disetujui";
                                        } else {
                                            echo $this->Html->changeStatusSelect($item['PaymentPurchase']['id'], ClassRegistry::init("VerifyStatus")->find("list", array("fields" => array("VerifyStatus.id", "VerifyStatus.name"))), $item['PaymentPurchase']['verify_status_id'], Router::url("/admin/payment_purchases/change_status_verify"), "#target-change-status$i");
                                        }
                                        ?>
                                    </td>
