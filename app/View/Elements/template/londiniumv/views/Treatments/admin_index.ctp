<?php
echo $this->element(_TEMPLATE_DIR . "/{$template}/filter/treatment");
?>
<div class="panel panel-default">
    <div class="panel-body">
        <div class="block-inner text-danger">
            <h6 class="heading-hr"><?= __("DATA TREATMENT (RETOUCHING)") ?>
                        ...
                        if (empty($data['rows'])) {
                            ...
                            foreach ($data['rows'] as $item) {
                                ...
                                <tr id="row-<?= $i ?>" class="removeRow<?php echo $item[Inflector::classify($this->params['controller'])]['id']; ?>">
                                    ...
                                    <td class="text-center"><?= $item['MaterialEntry']['Supplier']['name'] ?></td>
                                    ...
                                    <td class="text-center"  id = "target-change-statuses<?= $i ?>">
                                        <?php
                                        if ($roleAccess['edit']) {
                                            if ($item['Treatment']['validate_status_id'] == 1) {
                                                echo $this->Html->changeStatusSelect($item['Treatment']['id'], ClassRegistry::init("ValidateStatus")->find("list", array("fields" => array("ValidateStatus.id", "ValidateStatus.name"))), $item['Treatment']['validate_status_id'], Router::url("/admin/treatments/change_status_validate"), "#target-change-statuses$i");
                                            } else {
                                    ...
                                    <td class="text-center">
                                        <?php
                                        if ($item['Treatment']['validate_status_id'] != 2) {
                                        ...
                                        ?>
                                        <a data-toggle="modal" data-target="#default-view-coop-transaction" role="button" href="<?= Router::url("/admin/popups/content?content=viewtreatment_index&id={$item[Inflector::classify($this->params['controller'])]['id']}") ?>"><button type="button" class="btn btn-default btn-xs btn-icon tip" title="Lihat Data"><i class="icon-eye7"></i></button></a>
                                    </td>
                                </tr>
                                <?php
                                $i++;
                                        ...
                                        <div class="panel-body">
                                            <div class="table-responsive stn-table">
                                                <div class="panel-heading" style="background:#2179cc">
                                                    <h6 class="panel-title" style=" color:#fff"><i class="icon-menu2"></i><?= __("Detail Data Styling") ?></h6>
                                                </div>
                                                <table width="100%" class="table table-hover table-bordered">                        
                                                    <thead>
                                                        <tr>
...
<script>
        ...
        $(".viewData").click(function () {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    viewDataFreezing(data.Freeze.id);
                    $('#target-installment').empty();

                    if (data.Treatment.operator_id != null && data.Treatment.operator_id != "") {
                    ...
                    if (data != null && data != '') {
                        ...
                        $(".totalWeightTreatment").val((totalWeightTreatment.toFixed(2)));
                        $(".totalDifferWeight").val((totalSelisihBerat.toFixed(2)));
    ...
    function viewDataFreezing(freezeId) {
        $.ajax({
            ...
            success: function (data) {
                if (data != null && data != '') {
                    ...
                    $.each(data.FreezeDetail, function (index, items) {
                        ...
                        var options = {
                            i: i,
                            detail: items.Product.Parent.name + " " + items.Product.name,
                            quantity: (items.weight),
                            reject_type: reject_type,
                        };
                        var rendered = Mustache.render(template, options);
