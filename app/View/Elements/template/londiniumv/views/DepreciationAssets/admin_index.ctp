<?php
echo $this->element(_TEMPLATE_DIR . "/{$template}/filter/depreciation-asset");
?>
                    ...
                    <tbody>
                        ...
                        if (empty($data['rows'])) {
                            ...
                            foreach ($data['rows'] as $item) {
                                ?>
                                <tr id="row-<?= $i ?>" class="removeRow<?php echo $item["DepreciationAsset"]['id']; ?>">
                                    <td class="text-center"><input type="checkbox" name="data[<?php echo Inflector::classify($this->params['controller']) ?>][checkbox][]" value="<?php echo $item["DepreciationAsset"]['id']; ?>"  id="checkBoxRow" class="styled checkboxDeleteRow" /></td>
                                    ...
                                    <td class="text-center">
                                        <a data-toggle="modal" data-target="#default-view-depreciation-asset" role="button" href="<?= Router::url("/admin/popups/content?content=viewdepreciationasset&id={$item[Inflector::classify($this->params['controller'])]['id']}") ?>"><button type="button" class="btn btn-default btn-xs btn-icon tip" title="Lihat Data"><i class="icon-eye7"></i></button></a>
                                    </td>
                                </tr>
                                <?php
                                $i++;
