                    ...
                    <div class="col-md-6">
                        ...
                        if (isset($this->request->query['select_TransactionEntry_id'])) {
                            $transactionEntry = ClassRegistry::init("TransactionEntry")->find("first", [
                                "conditions" => [
                                    "TransactionEntry.id" => $this->request->query['select_TransactionEntry_id']
                                ],
                            ]);
                        }
                        ?>
                        ...
                        <div class="has-feedback">
                            <input type="text" placeholder="Silahkan Cari Nomor Transaksi ..." class="form-control typeahead-ajax-transaction" value="<?= !empty($transactionEntry) ? $transactionEntry['TransactionEntry']['transaction_number'] : "" ?>">
                            <input type="hidden" name="select.TransactionEntry.id" id="transactionEntry">
            ...
            <div class="form-group">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <?= $this->Form->input(null, ["default" => isset($this->request->query['select_TransactionEntry_supplier_id']) ? $this->request->query['select_TransactionEntry_supplier_id'] : '', "name" => "TransactionEntry.supplier_id", "class" => "select-full", "div" => false, "label" => false, "options" => $suppliers, "empty" => "", "placeholder" => "- Pilih Supplier -"]) ?>
                    </div>
                    <div class="col-md-6">
                        <label><?= __("Status") ?></label>
                        <?= $this->Form->input(null, ["default" => isset($this->request->query['select_TransactionEntry_transaction_entry_status_id']) ? $this->request->query['select_TransactionEntry_transaction_entry_status_id'] : '', "name" => "TransactionEntry.transaction_entry_status_id", "class" => "select-full", "div" => false, "label" => false, "options" => $statuses, "empty" => "", "placeholder" => "- Pilih Status -"]) ?>
                    </div>
                </div>
            </div>    
            <div class="form-group">
                <div class="row">
                    <div class="col-md-6">
                        <div class="has-feedback">
                            <?php
                            $employees = "";
                            if (isset($this->request->query['TransactionEntry_employee_id'])) {
                                $employees = ClassRegistry::init("Employee")->find("first", [
                                    "conditions" => [
                                        "Employee.id" => $this->request->query['TransactionEntry_employee_id']
                                    ],
                                    "contain" => [
                                        "Account" => [
                                            "Biodata"
                            ...
                            <input type="hidden" name="TransactionEntry.employee_id" id="employee">
                    ...
                    <div class="col-md-6">
                        <div class="has-feedback">
                            <?php
                            $creator = "";
                            if (isset($this->request->query['MaterialEntry_employee_id'])) {
                                $creator = ClassRegistry::init("Employee")->find("first", [
                                    "conditions" => [
                                        "Employee.id" => $this->request->query['MaterialEntry_employee_id']
                                    ],
                                    "contain" => [
                                        "Account" => [
                                            "Biodata"
                            ...
                            ?>
                            <label><?= __("Nama Pembuat") ?></label>
                            <input type="text" placeholder="Cari Nama Pegawai ..." class="form-control typeahead-ajax-creator" value="<?= !empty($creator) ? $creator['Account']['Biodata']['full_name'] : "" ?>">
                            <input type="hidden" name="MaterialEntry.employee_id" id="creator">
                            <i class="icon-search3 form-control-feedback"></i>
                        </div>
                    </div> 
                </div>
...
<script>
    ...
    $(document).ready(function () {
        var transaction = new Bloodhound({
            ...
            prefetch: '<?= Router::url("/admin/transaction_entries/list_normal", true) ?>',
            remote: {
                url: '<?= Router::url("/admin/transaction_entries/list_normal", true) ?>' + '?q=%QUERY',
                wildcard: '%QUERY',
            }
        });
        ...
        $('input.typeahead-ajax-transaction').bind('typeahead:select', function (ev, suggestion) {
            ...
            $('#transactionEntry').val(suggestion.id);
...
<script>
    ...
    creator.clearPrefetchCache();
    creator.initialize(true);
    $('input.typeahead-ajax-creator').typeahead({
        hint: false,
        highlight: true
    }, {
        name: 'employee',
        display: 'full_name',
        source: creator.ttAdapter(),
        templates: {
            header: '<center><h5>Data Pegawai</h5></center><hr>',
            suggestion: function (data) {
                return '<p> Nama : ' + data.full_name + '<br/> NIP Pegawai : ' + data.nip + '<br/> Department : ' + data.department + '<br/> Position : ' + data.jabatan + '</p>';
    ...
    $('input.typeahead-ajax-creator').bind('typeahead:select', function (ev, suggestion) {
        $("#creator").val(suggestion.id);
    });
</script>