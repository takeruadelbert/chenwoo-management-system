                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="has-feedback">
                            <?php
                            $employee = "";
                            if (isset($this->request->query['select_EmployeeBalance_employee_id'])) {
                                $employee = ClassRegistry::init("Employee")->find("first", [
                                    "conditions" => [
                                        "Employee.id" => $this->request->query['select_EmployeeBalance_employee_id']
                                    ],
                                    "contain" => [
                                        "Account" => [
                                            "Biodata"
                            ...
                            <input type="hidden" name="select.EmployeeBalance.employee_id" id="employee">
                            <i class="icon-search3 form-control-feedback"></i>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <label><?= __("Nomor Rekening") ?></label>
                        <?= $this->Form->input(null, ["div" => false, "label" => false, "class" => "form-control tip", "type" => "text", "name" => "EmployeeBalance.account_number", "default" => isset($this->request->query['EmployeeBalance_account_number']) ? $this->request->query['EmployeeBalance_account_number'] : ""]) ?>
                    </div>
                </div>
            </div>
